...
conversations_ids = []
for conversation in conversations[:-1]:   ## Except the last Conversation
...
# Creating the dictionaries that maps the questions words and the answers word to a unique integer
thereshold = 20 # set it for the minimum occurence of the word
questionswords2int = {}
word_number = 0;
...
for token in tokens:        
    questionswords2int[token] = len(answerwords2int) + 1
for token in tokens:        
...
#Adding the End Of the String token to the End of each Answer

for i in range(len(clean_answers)):
    clean = re.sub("<EOS>","",clean_answers[i])
    clean_answers[i] = clean
...
while(True):
    ...
    fake_batch[1] = question
    predicted_answer = session.run(test_predictions, {inputs: fake_batch, keep_prob: 0.5})[0]
   # print(predicted_answer.shape)
    count = 0;
    answer = ''
    for i in np.argmax(predicted_answer, axis = 0):
